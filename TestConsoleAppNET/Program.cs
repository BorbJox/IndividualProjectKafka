    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var producer = new ProducerBuilder<string, string>(configuration.GetSection("KafkaSettings").AsEnumerable(true)).Build())
            {
                ...
                Console.WriteLine("Generating bets... Press Ctrl+C to stop.");
                try
                {
                    while (true)
                    {
                        for (int i = 0; i < 1; i++)
                        {
                            ...
                            producer.Produce("placed_bets", new Message<string, string> { Key = bet.Id.ToString(), Value = jsonString },
                                (deliveryReport) =>
                                {
                                    ...
                                    {
                                        Console.WriteLine($"Produced event to topic placed_bets: key = {bet.Id,-10} value = {jsonString}");
                                        numProduced += 1;
                                    }
                                });
                ...
                finally
                {
                    ...
                    Console.WriteLine($"{numProduced} messages were produced to topics");
                }
            }
        }
